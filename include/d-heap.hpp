...
class MyPow {
public:
	static powType pow(const powType& foundation, size_t exponent){
		powType result = 1;
...
template <class HType>
void DHeap<HType>::print()
{
	int level = 0, tmp = 0;
	while (tmp < kolvo)
	{
		tmp += MyPow<int>::pow (d,level); 
		level++;
	}
	int k = 1, z = 0;
	cout << "\t" << keys[0] -> pr << endl;
	...
	{
		for (int j = k; (j < k + MyPow<int>::pow(d,i)) && (j < kolvo); j++)
			cout << keys[j] -> pr << "\t";
		cout << endl;
		if (k + MyPow<int>::pow(d,i) > kolvo)
				k = kolvo;
			else k += MyPow<int>::pow(d,i);
	}
}
