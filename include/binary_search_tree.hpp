...
public:
	...
	TType key; //ключ
	Node<TType> *left; //ук-ль на лево
	Node<TType> *right; //ук-ль на право
...
Node<TType>* bintree<TType>::Copytree (Node<TType> *rot)
{
	...
	Node<TType>* tree = new Node<TType>;
	tree -> key = rot -> key;
	tree -> left = l;
	tree -> right = r;
	root = tree;
...
Node<TType>* bintree<TType>::FindNext (Node<TType> *tr, Node<TType> *node)
{
	if (tr == NULL)
		throw
		exception("error: tree is empty");
	if (node == NULL)
		throw
		exception("error: key is empty");
	...
	if (node -> parent == NULL)
		throw
		exception("error: parent is absent");
