...
void DQueue<TType>::pop()
{
	if (isEmpty())
		throw
		exception ("error: queue is empty");
	heap -> delet();
}
...
TQueue<TType>::TQueue (int a)
{
	if (a < 0)
		throw 
		exception ("error: incorrect size of table");
...
template <class TType>
bool make_queue<TType>::makeQueue (PQueue<TType>*& queue, const int a, const int b)
{
	if (a == 1)
	{
		queue = new DQueue<TType>(b);
		return true;
	}
	if (a == 2)
	{
		queue = new BQueue<TType>();
		return true;
	}
	if (a == 3)
	{
		queue = new TQueue<TType>(b);
		return true;
	}
	return false;
}
